...
params = {
	# Define the system
	'boundary': ('f','f','p'),
	'box':  (-1e-3, 1e-3, -1e-3, 1e-3, -14e-3, 3.5e-3),
	...
	# Import hopper + impeller mesh
	 'mesh': {
		...
		'sphere': {'file': 'mesh/sphere.stl', 'mtype': 'mesh/surface', 'material': glass, \
                'args': {'heal': 'auto_remove_duplicates', 'move': (0, 0, 0.7e1), 'scale': 2.75e-4}},
        'tube': {'file': 'mesh/tube.stl', 'mtype': 'mesh/surface', 'material': glass, \
                'args': {'heal': 'auto_remove_duplicates', 'move': (0, 0, -25e1), 'scale': 3.7e-5}},
	},

	# Stage runs
	'stages': {'insertion': 1e5, 'run': 10e5},   # I think 10e5 will be a good run number
}
...
insert = sim.insert(species=1, region=('block', -5e-4, 5e-4, -5e-4, 5e-4, -10e-3, 3e-3), \
                    mech='volumefraction_region', value=1, freq=1e4)
#insert = sim.insert(
 # species=1,
 # region=('block', -5e-4,5e-4, -5e-4,5e-4, 0,3e-3),
 # mech='particles_in_region',
 # value=1000,     # insert 1000 particles per insertion
 # freq=1e4
#)
