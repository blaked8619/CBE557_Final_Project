...
def get_z_positions():
    # grab the raw LAMMPS compute buffer
    buf = sim.extract_compute(
        'zpos',
        LMP_STYLE_ATOM,      # per-atom data
        LMP_TYPE_VECTOR      # vector per atom
    )
    # turn it into a memoryview, cast to double (C 'd')
    
    n_atoms = sim.get_natoms()
    z_positions = np.frombuffer(buf, dtype=np.float64, count=n_atoms)
    # wrap that in a NumPy array
    return z_positions
